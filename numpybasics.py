...
diff = np.diff(newArray)   # calculates the difference between subsequent values in an array
print(diff)

cummsum = np.cumsum(np.arange(5))   # cummulative sum of array elements [0,1,2,3,4,5] - note new array shape
...
argsorted = np.argsort(rando)   # returns the index of the values if they were sorted in ascending order... cool!
print(argsorted)
...
print(np.nonzero(sorto), sorto[np.nonzero(sorto)])  # will return the non-zero indices, values
