...
filtered = filter(lambda planes: True if planes < 4 else False, planeQuantities)    # removes false values
print(list(filtered))

zipped = list(zip(planeQuantities, planeTypes))   # combines two sequences like a zipper - (0,0), (1,1) etc
print(zipped)
...
dict = {'y':0, 'x':1, 'z':4}    # note strings need to be the argument variables, but not in order
print(func(**dict))
